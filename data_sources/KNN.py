...
def knn(X_train,y_train, X_test, votes,n_neighb):
    #construction +entrainement
    knnclassifier = KNeighborsClassifier(n_neighbors= n_neighb, metric ="manhattan")
...
def start_KNN(data_path, train_path, test_path,n_neighb,
    ...
    for u in users:
        ...
        # pred = KNN(X_train, y_train, X_test, y_test, votes)
        ...
        if len(y_train.unique()) != 2:
            seul += 1
            continue
        pred = knn(X_train, y_train, X_test, votes, n_neighb)
        taille += 1
    ...
    name = "KNN_"+"_moy_prec.pkl"
    with open(name,'wb') as f:
        p = pickle.Pickler(f)
        p.dump(moy_p)
        
    name = "KNN_"+"_moy_rec.pkl"    
    with open(name,'wb') as f:
        p = pickle.Pickler(f)
        p.dump(moy_r)  
