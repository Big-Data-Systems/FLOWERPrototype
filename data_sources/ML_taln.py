...
def cbf_language_model(profile, query):
    ...
    stemmer = Stemmer()
    stem_profile = [stemmer.stem(w) for w in token_profile if w not in stopwords.words("english")]
    stem_query = [stemmer.stem(w) for w in token_query if w not in stopwords.words("english")]
